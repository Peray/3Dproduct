...
(function($) {
    ...
    function Slider($element) {
        ...
        this.visibleGridWidth = ($(window).width()) * 0.9 / 12,
            this.visibleColumnsDivvy = 6;
        this.monthNames = ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一", "十二"];
    ...
    Slider.prototype.makeGrid = function(isDivvy) {
        ...
        TweenLite.set(this.track, {
            height: this.gridRows * this.gridHeight + 1,
            width: this.gridColumns * this.gridWidth *0.9 + 1,
            margin:'78px auto 0'
        });
    ...
    Slider.prototype.setSliderDate = function(xPos) {
        ...
        var allx=this.gridWidth*xPos;
        var xpre = Math.max(1, Math.round(allx/ (this.gridWidth*0.9)));

        var date = this.lookupDate(xpre);

        var datePieces = String(date).split(" ");
        var month = datePieces[1];
        var dayYear = datePieces[1] + ' ' + datePieces[3];

        var htmlString = datePieces[2] + '日 <br /><span class="txt-Day">' + datePieces[3] + '</span>';
