...
public class UserSessionFilter implements Filter {
	...
	public void doFilter(ServletRequest arg0, ServletResponse arg1,
		...
		//不拦截的页面
		String[] except = {"/", "/index.jsp", "/logon.jsp", "first.jsp","/third.jsp","/secondly.jsp","/read.jsp"};
...
//		logger.info("currentURL：" + currentURL + "   targetURL: " + targetURL);
		...
		if (!currentURL.contains("/admin") && !currentURL.contains("/example")) {
			if (!exceptSet.contains(targetURL)) {
				...
				if (session.getAttribute(StaticName.session_user) == null) {
					...
					logger.info("session失效，当前url：" + currentURL + "   重定向到url: " + targetURL);
