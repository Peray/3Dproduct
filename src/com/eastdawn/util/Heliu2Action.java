...
//                 	hupo.setShapeArea(info.get("Shape_Area").toString());
                     ...
                     System.out.println("[" + i + "]LAKE_CODE=" + info.get("HYDC").toString());  
                   ...
                   //3、向数据库中插入一条数据   
                     String sql = "INSERT INTO tb_heliu_two(geometry,gb,hydc,shape_Leng,object_Id,name) values ('"+jsonObject.get("geometry").toString()+"','"+info.get("GB").toString()+"','"+info.get("HYDC").toString()+"','"+info.get("Shape_Leng").toString()+"','"+info.get("OBJECTID").toString()+"','"+info.get("NAME").toString()+"')";   
                     try {   
